...
static FileCrypt *s_FileCrypt = nullptr;

FileCrypt* FileCrypt::getInstance()
{
	if (!s_FileCrypt)
	{
		s_FileCrypt = new (std::nothrow) FileCrypt;
	}
...
void FileCrypt::cleanupXXTEAKeyAndSign()
{
	if (_xxteaKey)
	{
		free(_xxteaKey);
...
void FileCrypt::setXXTEAKeyAndSign(const char *key, int keyLen, const char *sign, int signLen)
{
	cleanupXXTEAKeyAndSign();
...
unsigned char* FileCrypt::decrypt(const unsigned char *data, int dataSize, unsigned int* originSize)
{
	if (_xxteaEnabled && strncmp((const char*)data, _xxteaSign, _xxteaSignLen) == 0)
	{
		// decrypt XXTEA
		unsigned char* result = xxtea_decrypt((unsigned char*)data + _xxteaSignLen,
			(xxtea_long)dataSize - _xxteaSignLen,
